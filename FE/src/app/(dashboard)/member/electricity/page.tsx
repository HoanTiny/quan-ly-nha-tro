      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-white rounded-2xl shadow p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-gray-500">Hôm qua</span>
            </div>
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-medium text-gray-500">So sánh</span>
            </div>
            <p
        ...
        <div className="bg-white rounded-2xl shadow overflow-hidden">
          <div className="px-4 py-3 bg-gradient-to-r from-gray-50 to-gray-100 border-b">
            <h2 className="text-sm font-semibold text-gray-700">Chi tiết chỉ số hàng ngày</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[500px]">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase whitespace-nowrap">Ngày</th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase whitespace-nowrap">Chỉ số</th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase whitespace-nowrap">Tiêu thụ</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {dailyUsage
                  ...
                  .map((item, idx) => {
                    ...
                    const currentIndex = baseIndex + dailyUsage.slice(0, dailyUsage.length - idx).reduce((acc, d) => acc + d.usage, 0);
                      ...
                      <tr key={idx} className={`hover:bg-gray-50 transition-colors ${isToday ? 'bg-orange-50' : ''}`}>
                        <td className="px-4 py-3">
                          <div>
                            ...
                            <p className="text-xs text-gray-400">{getDayOfWeek(item.date)}</p>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <p className="text-sm font-semibold text-gray-700">{(currentIndex).toFixed(0)}</p>
                        </td>
                        <td className="px-4 py-3 text-right">
